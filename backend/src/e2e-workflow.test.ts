...
describe('VendorBridge End-to-End Procurement Flow', { timeout: 60000 }, () => {
  ...
  it('Step 1: Authentication & JWT Login for all roles', async () => {
    ...
    const offRes = await app.inject({
      method: 'POST',
      url: '/api/auth/login',
      payload: { email: 'e2e_officer@vendorbridge.com', password: 'Password123!' },
    });
    ...
    const mgrRes = await app.inject({
      method: 'POST',
      url: '/api/auth/login',
      payload: { email: 'e2e_manager@vendorbridge.com', password: 'Password123!' },
    });
  ...
  it('Step 3: Vendor 1 saves draft quotation, then submits final bid', async () => {
    ...
    const draftRes = await app.inject({
      ...
      payload: {
        ...
        paymentTerms: 'Net 30',
        items: [
          { rfqItemId: items[0].id, name: items[0].name, quantity: 2, unitPrice: 180000, tax: 32400 },
          { rfqItemId: items[1].id, name: items[1].name, quantity: 4, unitPrice: 45000, tax: 16200 },
        ],
      },
    });
  ...
  it('Step 4: Vendor 2 submits a competing bid', async () => {
    ...
    const quote2Res = await app.inject({
      ...
      payload: {
        ...
        paymentTerms: 'Net 45',
        items: [
          { rfqItemId: items[0].id, name: items[0].name, quantity: 2, unitPrice: 195000, tax: 35100 },
          { rfqItemId: items[1].id, name: items[1].name, quantity: 4, unitPrice: 42000, tax: 15120 },
        ],
      },
    });
  ...
  it('Step 6: Officer shortlists winning quote, creating Approval request for Manager', async () => {
    ...
    await prisma.rFQ.update({
      ...
      data: { status: 'CLOSED', quotationDeadline: new Date(Date.now() - 1000) },
    });
  ...
  it('Step 7: Finance Manager reviews and approves the quotation', async () => {
    ...
    const appRes = await app.inject({
      ...
      headers: { authorization: `Bearer ${managerToken}` },
      payload: { comment: 'Approved. Pricing is competitive and within allocated FY26 budget.' },
    });
