...
import { ApprovalQuery, ApprovalDecisionInput, CreateApprovalInput } from "./approval.schema";
...
export class ApprovalService {
  ...
  async get(user: SafeUser, id: number) {
    ...
    if (!approval) {
      throw AppError.notFound("Approval request not found", "APPROVAL_NOT_FOUND");
    }
    return approval;
  }
  ...
  async create(user: SafeUser, input: CreateApprovalInput) {
    ...
    const created = await prisma.$transaction(async (tx) => {
      await quotationRepository.updateStatus(input.quotationId, "SHORTLISTED", tx);
      await rfqRepository.update(input.rfqId, { status: "AWAITING_APPROVAL" }, tx);
      const appReq = await approvalRepository.create(
        { rfqId: input.rfqId, quotationId: input.quotationId, requestedById: user.id },
        tx
      );
  ...
  async approve(user: SafeUser, id: number, input: ApprovalDecisionInput) {
    ...
    if (approval.status !== "PENDING") {
      throw AppError.conflict(`Approval request is already ${approval.status.toLowerCase()}`, "APPROVAL_ALREADY_DECIDED");
    }
    ...
    await prisma.$transaction(async (tx) => {
      await approvalRepository.updateStatus(
        id,
        { status: "APPROVED", approvedById: user.id, decidedAt: now, comment: input.comment },
        tx
      );
      await quotationRepository.updateStatus(approval.quotationId, "APPROVED", tx);
      await rfqRepository.createActivity(
        approval.rfqId,
        user.id,
        "APPROVAL_DECISION",
  ...
  async reject(user: SafeUser, id: number, input: ApprovalDecisionInput) {
    if (!input.comment || !input.comment.trim()) {
      throw AppError.badRequest("Rejection remarks are required", "REMARKS_REQUIRED");
    }
    ...
    await prisma.$transaction(async (tx) => {
      await approvalRepository.updateStatus(
        id,
        { status: "REJECTED", approvedById: user.id, decidedAt: now, comment: input.comment },
        tx
      );
      await quotationRepository.updateStatus(approval.quotationId, "REJECTED", tx);
      await rfqRepository.update(approval.rfqId, { status: "UNDER_REVIEW" }, tx);
      await rfqRepository.createActivity(
        approval.rfqId,
        user.id,
        "QUOTATION_REJECTED",
