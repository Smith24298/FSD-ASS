    ...
    try {
      ...
      if (quotationId) {
        ...
        if (
          ...
          Number(savedQuotation.rfqId) !== Number(rfqId)
        ) {
      ...
      setItems(
        (data?.items || []).map((it: any) => {
          const savedItem = savedQuotation?.items?.find(
            (item: any) => item.rfqItemId === it.id,
          );
          return {
            rfqItemId: it.id,
            name: it.name,
      ...
      if (savedQuotation) {
        ...
        setPaymentTerms(
          savedQuotation.paymentTerms || "Net 30 Days from Invoice",
        );
    ...
    try {
      const payload = {
        ...
        deliveryDays: deliveryDays
          ? parseInt(deliveryDays.toString(), 10)
          : undefined,
        validityDays: validityDays
          ? parseInt(validityDays.toString(), 10)
          : undefined,
        notes,
        items: items.map((it) => ({
    ...
    <div>
      ...
      <div className="page-header">
        <div>
          ...
          >
            Quote pricing, delivery timelines, and payment terms for RFQ:{" "}
            ...
            >
              <div className="form-group">
                ...
                <input
                  ...
                  value={deliveryDays}
                  onChange={(e) =>
                    setDeliveryDays(parseInt(e.target.value, 10))
                  }
                />
              </div>
              <div className="form-group">
                ...
                <input
                  ...
                  value={validityDays}
                  onChange={(e) =>
                    setValidityDays(parseInt(e.target.value, 10))
                  }
                />
              </div>
            </div>
        ...
        >
          <div className="card" style={{ padding: "1.5rem" }}>
            ...
            >
              ...
              >
                <Save size={16} />{" "}
                {existingQuotation ? "Save Draft Changes" : "Save as Draft"}
              </button>
            </div>
          </div>
        </div>
