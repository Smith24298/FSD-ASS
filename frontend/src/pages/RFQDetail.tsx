...
import {
  Package, ArrowLeft, Calendar, FileText, CheckCircle2,
  Clock, AlertCircle, Send, Plus, Eye, Award
} from 'lucide-react';
...
export default function RFQDetail() {
  ...
  const handleAward = async (quotationId: number) => {
    if (!confirm('Are you sure you want to award this RFQ to this quotation? This will create a Purchase Order.')) return;
    setActionLoading(true);
    try {
  ...
  if (isLoading) {
    return <div style={{ padding: '2rem', textAlign: 'center' }}>Loading RFQ details...</div>;
  }
  ...
  const isDeadlinePassed = rfq.quotationDeadline && new Date(rfq.quotationDeadline) < new Date();

  return (
    <div>
      <div style={{ marginBottom: '1.5rem' }}>
        <button className="btn btn-secondary" onClick={() => navigate('/rfqs')} style={{ marginBottom: '0.75rem' }}>
          <ArrowLeft size={16} /> Back to RFQs
        </button>
        <div className="page-header" style={{ marginBottom: 0 }}>
          <div>
            ...
            <div style={{ fontFamily: 'monospace', fontSize: '0.85rem', color: '#6B7280', marginTop: '0.25rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
            {user?.role === 'OFFICR' && rfq.status === 'DRAFT' && (
              <button className="btn btn-primary" onClick={handlePublish} disabled={actionLoading}>
                <Send size={16} /> Publish RFQ
              </button>
            )}

            {user?.role !== 'VENDOR' && (rfq.status === 'OPEN' || rfq.status === 'PUBLISHED' || rfq.status === 'SHORTLISTED') && (
              <Link to={`/rfqs/${rfq.id}/compare`} className="btn btn-primary">
                Compare Quotations
              </Link>
            )}

            {user?.role === 'VENDOR' && (rfq.status === 'OPEN' || rfq.status === 'PUBLISHED') && !myQuotation && !isDeadlinePassed && (
              <Link to={`/rfqs/${rfq.id}/quote`} className="btn btn-success">
                <Plus size={16} /> Submit Quotation
              </Link>
            )}

            {user?.role === 'VENDOR' && myQuotation && myQuotation.status === 'DRAFT' && !isDeadlinePassed && (
              <Link to={`/rfqs/${rfq.id}/quote?quotationId=${myQuotation.id}`} className="btn btn-success">
                <Send size={16} /> Edit Draft Quotation
              </Link>
            )}

            {user?.role === 'VENDOR' && myQuotation && myQuotation.status !== 'DRAFT' && (
              <Link to={`/quotations/${myQuotation.id}`} className="btn btn-secondary">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div className="card" style={{ padding: '1.5rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '1rem', marginTop: '1.25rem', borderTop: '1px solid #F3F4F6', paddingTop: '1rem' }}>
              <div>
                <div style={{ fontSize: '0.75rem', color: '#6B7280', textTransform: 'uppercase', fontWeight: 500 }}>Quotation Deadline</div>
                <div style={{ fontSize: '0.9rem', fontWeight: 600, color: isDeadlinePassed ? '#EF4444' : '#111827', marginTop: '0.2rem' }}>
                  {rfq.quotationDeadline ? new Date(rfq.quotationDeadline).toLocaleString() : 'Open'}
                  {isDeadlinePassed && ' (Passed)'}
                </div>
              </div>
              <div>
                <div style={{ fontSize: '0.75rem', color: '#6B7280', textTransform: 'uppercase', fontWeight: 500 }}>Target Delivery</div>
              ...
              <div>
                <div style={{ fontSize: '0.75rem', color: '#6B7280', textTransform: 'uppercase', fontWeight: 500 }}>Estimated Budget</div>
                <div style={{ fontSize: '0.9rem', fontWeight: 600, color: '#111827', marginTop: '0.2rem' }}>
              ...
              <tbody>
                {rfq.items?.map((item: any) => (
                  <tr key={item.id}>
                    ...
                    <td style={{ color: '#6B7280', fontSize: '0.85rem' }}>{item.description || '—'}</td>
                    <td style={{ fontWeight: 600 }}>{item.quantity}</td>
                    <td>{item.unit || 'Units'}</td>
                    <td>{item.estimatedUnitPrice ? `₹${Number(item.estimatedUnitPrice).toLocaleString()}` : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div className="card" style={{ padding: '1.5rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                ...
                {rfq.quotations?.length > 1 && (
                  <Link to={`/rfqs/${rfq.id}/compare`} className="btn btn-primary" style={{ fontSize: '0.8rem' }}>
              ...
              {(!rfq.quotations || rfq.quotations.length === 0) ? (
                <p style={{ color: '#6B7280', fontSize: '0.875rem', margin: 0 }}>No quotations submitted yet.</p>
              ) : (
                    ...
                    <div key={q.id} style={{
                      ...
                      <div>
                        <div style={{ fontWeight: 600, fontSize: '0.9rem', color: '#111827' }}>
                          {q.vendor?.name || 'Vendor'} — ₹{Number(q.totalAmount).toLocaleString()}
                        </div>
                        <div style={{ fontSize: '0.75rem', color: '#6B7280', marginTop: '0.2rem' }}>
                          Quote #{q.quotationNumber || q.id} • Delivery: {q.deliveryDays || 'N/A'} days • Submitted: {new Date(q.submittedAt || q.createdAt).toLocaleDateString()}
                        </div>
                      </div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                        ...
                        {user?.role === 'OFFICR' && rfq.status === 'SHORTLISTED' && q.status === 'APPROVED' && (
                          <button
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div className="card" style={{ padding: '1.5rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {rfq.vendors?.map((inv: any) => (
                  <div key={inv.vendorId} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.5rem 0', borderBottom: '1px solid #F3F4F6' }}>
                    <div>
                      <div style={{ fontSize: '0.85rem', fontWeight: 600 }}>{inv.vendor?.name}</div>
                      <div style={{ fontSize: '0.75rem', color: '#6B7280' }}>{inv.vendor?.email}</div>
                    </div>
          ...
          <div className="card" style={{ padding: '1.5rem' }}>
            ...
            {activities.length === 0 ? (
              <p style={{ color: '#6B7280', fontSize: '0.8rem', margin: 0 }}>No timeline activities logged yet.</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {activities.map((act) => (
                  <div key={act.id} style={{ display: 'flex', gap: '0.75rem', fontSize: '0.8rem' }}>
                    ...
                    <div>
                      <div style={{ fontWeight: 600, color: '#111827' }}>{act.event.replace(/_/g, ' ')}</div>
                      <div style={{ color: '#4B5563', marginTop: '0.1rem' }}>{act.description}</div>
